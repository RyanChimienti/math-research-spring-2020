...
W_{ij} = 
\begin{cases}
\frac{\sigma_{ji}}{d_j} &\text{ if } i \sim j \\
0 &\text{ if } i \not\sim j
\end{cases}
$$
for all $i, j \in V$.
...
It may feel more natural to write $\sigma_{ij}$ in our definition rather than $\sigma_{ji}$, and this wouldn't cause significant problems, but our choice better captures the meaning of the PageRank matrix. Given a vector $v \in \C^n$ which represents PageRank values for each vertex, performing the multiplication $W v$ should cause the vertices to distribute their PageRank evenly among their neighbors. However, since we are dealing with magnetic graphs, some rotation should be gained each time PageRank is transferred along a magnetized edge. If we want that gained rotation to be the signature of the edge \textbf{in the direction of the transfer}, then we must use $\sigma_{ji}$ in our definition, and not $\sigma_{ij}$. (Our choice is further justified by the later \cref{Laplacian Eigenvectors vs PageRank Eigenvectors}, which would pick up an extra transpose (on $L$) if we had used the other definition.)
...
&= \sum_{j=1}^n \left( \abs*{x_j} \sum_{i \sim j} \abs*{\frac{\sigma_{ji}}{d_j}} \right)
= \sum_{j=1}^n \left( \abs*{x_j} \frac{1}{d_j} \sum_{i \sim j} \abs*{\sigma_{ji}} \right) 
...
Let $G$ be a magnetic graph on $n$ vertices, with PageRank matrix $W$ and Laplacian $L$. Let $x \in \C^n$. Then $W^T x = x$ if and only if $L x = 0$.
\end{prop}
\begin{proof}
Let $A$ and $D$ be the adjacency matrix and degree matrix of $G$, respectively.  Then $W = A^T D^{-1}$ and $L = D - A$. So it follows that
\begin{align*}
W^T x = x &\iff (A^T D^{-1})^T x = x \\
&\iff D^{-1} A x = x \\
&\iff A x = D x \\
&\iff (D - A) x = 0 \\
&\iff L x = 0.
...
&\iff &&\text{There exists a nonzero vector } x \in \C^n \text{ satisfying } W^T x = x \\
&\iff &&\text{There exists a nonzero vector } x \in \C^n \text{ satisfying } L x = 0 &\quad &\text{(by \cref{Laplacian Eigenvectors vs PageRank Eigenvectors})} \\
...
\begin{defn}
Let $W$ be an $n \times n$ PageRank matrix. Then the \textit{personalized PageRank vector} with \textit{jumping constant} $\alpha \in (0, 1)$ and \textit{seed vector} $s \in \C^n$, written pr$(\alpha, s)$, is defined to be the unique $x \in \C^n$ satisfying
$$
...
$$ 
This is maybe the most straightforward way to see that our definition of the personalized PageRank vector is valid, but we can alternatively use \cref{inverse of I - M} to get that $(I - (1 - \alpha)W)$ is invertible and 
$$
...
\begin{prop}
Let $W$ be an $n \times n$ PageRank matrix, $\alpha \in (0, 1)$, and $s \in \C^n$. Then $$\text{pr}(\alpha, s) = \alpha \left( \sum_{k=0}^{\infty} (1 - \alpha)^k W^k \right) s.$$
\end{prop}
