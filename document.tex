...
Throughout this section, let $G$ be a magnetic graph on $n$ vertices, with degree matrix $D$ and adjacency matrix $A$. Assume $G$ is connected and has at least 2 vertices, so that $D$ is invertible. Let $G$ have Laplacian $L$, PageRank matrix $W$, and lazy PageRank matrix $Z$. Fix $\alpha \in (0, 1)$ to be used as a jumping constant when computing the personalized PageRank vector for $G$, and let $$\beta := \frac{2 \alpha}{1 - \alpha}.$$ We now describe the normalized Laplacian and some of its properties.
\begin{defn} 
...
$$
\end{defn}
\begin{prop}\label{Normalized Laplacian is Hermitian}
$\lap$ is Hermitian.
\end{prop}
\begin{proof}
We have:
$$
\lap^* = \left( D^{-1/2} L D^{-1/2} \right)^* = D^{-1/2} L^* D^{-1/2} = D^{-1/2} L D^{-1/2} = \lap.
$$
\end{proof}

We take the following definition from Wikipedia and Linear Algebra Done Right.

\begin{defn}
A matrix $A \in M_{n \times n}(\C)$ is said to be \textit{positive-semidefinite} if $A$ is Hermitian and $$\inner*{ Av, v } \geq 0$$ for every $v \in \C^n$. (Note that $\inner*{ Av, v}$ is automatically real since $A$ is Hermitian.)
\end{defn}

\begin{prop}
$\lap$ is positive-semidefinite.
\end{prop}
\begin{proof}
$\lap$ is Hermitian by \cref{Normalized Laplacian is Hermitian}. Now let $v \in \C^n$. We have:
\begin{align*}
\inner*{ \lap v, v } &= \inner*{ D^{-1/2} L D^{-1/2} v, v } \\
&= \inner*{ L D^{-1/2} v, D^{-1/2} v } && \text{(because $D^{-1/2}$ is self-adjoint)} \\
&= \inner*{ L \left( D^{-1/2} v \right), D^{-1/2} v } \\
&\geq 0 && \text{(because $L$ is positive-semidefinite by \cref{Laplacian is positive semi-definite})}.
\end{align*}
\end{proof}

Now from Linear Algebra Done Right (Proposition 7.35, pg 226) we have:

\begin{prop}
Let $A \in M_{n \times n}(\C)$. Then $A$ is positive-semidefinite iff $A$ is Hermitian and all the eigenvalues of $A$ are nonnegative.
\end{prop}

So we can conclude that $\lap$ has $n$ real, nonnegative eigenvalues. Going forward, we will refer to these as 
...
Next we want to define an ``asymmetric" (or in other words, non-Hermitian) version of the Green's Function. Asymmetrizing the Green's function will help us relate it to PageRank, since as we shall see the PageRank matrix $W$ is, in some sense, an asymmetric version of $\lap$. The following definition breaks the Green's function's symmetry while preserving its conjugacy class.
...
We mentioned earlier that $W$ is like an asymmetric version of $\lap$. To be specific, we have the following relationship.

\begin{prop}\label{Laplace operator and PageRank}
We have: $$\laplace = I - W.$$
\end{prop}
\begin{proof}
$$\laplace = D^{1/2} \lap D^{-1/2} = L D^{-1} = (D - A)D^{-1} = I - W.$$
\end{proof}
