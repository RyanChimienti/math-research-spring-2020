...
We now turn our attention to the PageRank vectors resulting from seed vectors of a special form. Namely, let $S$ be an induced subgraph of $G$, and define the seed vector $f_S \in \C^n$ by
$$
(f_S)_v =
\begin{cases}
...
$$
\lazypr(\alpha, f_S)(S) \geq 1 - \frac{\abs*{\partial(S)} + \sum_{(u,v) \in E^{or}(G)}{(1 - \Re(\sigma_{uv}))}}{\beta \vol (S)}.
$$
\end{thm}
