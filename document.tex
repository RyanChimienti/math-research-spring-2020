...
\begin{corollary}
If $c$ is a switching function for $G$ with $c_u = c_v$, then $R^c (u, v) = R(u, v)$.  
\end{corollary}
...
\begin{corollary}
If $c$ is a switching function for $G$ with $c_u = c_v$, then $H^c (u, v) = H(u, v)$.  
\end{corollary}
...
$$
H(u, v) = \asd(u, v) + \sum_{w \sim u} H^w(w, v)
$$
where $H^w(w, v)$ denotes the hitting time from $w$ to $v$, computed on a switched version of $G$ where the switching function takes // TODO
\end{thm}
